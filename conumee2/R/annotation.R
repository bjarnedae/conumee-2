...
CNV.create_anno <- function(bin_minprobes = 15, bin_minsize = 50000, bin_maxsize = 5e+06,
  ...
  #mouse beginning
    ...
    ao_probes <- GRanges(as.vector(seqnames(probes)), ranges(probes),
        seqinfo = Seqinfo(object@genome$chr, object@genome$size))
    ao_probes$genes <- probes$genes
    ao_probes <- sort(ao_probes)
    object$probes <- ao_probes
    message(" - ", length(object@probes), " probes used")
...
CNV.merge_bins_mice <- function(hg19.anno, hg19.tile, bin_minprobes = 20, hg19.probes,
                           bin_maxsize = 5e+06, verbose = FALSE) {
  values(hg19.tile)$probes <- countOverlaps(hg19.tile, hg19.probes)
  hg19.tile.df <- as.data.frame(hg19.tile)[, c("seqnames", "start", "end",
                                               "probes")]
  ...
  names <- formatC(unlist(lapply(table(seqnames(hg19.tile.bin)),
                        seq)), width = nchar(max(table(seqnames(hg19.tile.bin)))), format = "d", flag = "0")
  names <- names[-c(8540:8543)]
  names(hg19.tile.bin) <- paste(as.vector(seqnames(hg19.tile.bin)),
                                names,
                                sep = "-")
  return(hg19.tile.bin)
}
